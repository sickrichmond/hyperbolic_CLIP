...
sys.path.append(os.path.join(current_dir,"robustness/ImageNet-C/imagenet_c"))

# `corrupt` is imported lazily, inside the two methods that call it. RepmixDataset
# builds these transforms but only ever applies transform[0] (prep) and
# transform[2] (norm) — the perturbation step is dead in the IAB pipeline — so a
# module-level import would make `imagenet_c` a hard dependency for code that
# never runs. Behaviour is unchanged; the dep is only needed if it is really used.
