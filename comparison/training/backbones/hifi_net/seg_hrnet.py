...
class HighResolutionModule(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.num_branches): # 分支上的转换
            x[i] = self.branches[i](x[i])

        x_fuse = []
        for i in range(len(self.fuse_layers)): #分支之间融合
            ...
            for j in range(1, self.num_branches):
                if i == j:
                    ...
                    y = y + F.interpolate(
                        self.fuse_layers[i][j](x[j]),#通过插值提升分辨率
                        size=[height_output, width_output],
                        mode='bilinear', align_corners=True)
                else:
                    y = y + self.fuse_layers[i][j](x[j]) # fused_layer已经包含了降分辨率的操作
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        # print(f"stage1 block: {self.stage1_cfg} ")
    ...
    def forward(self, x):
        ...
        for i in range(self.stage2_cfg['NUM_BRANCHES']):
            if self.transition1[i] is not None:
                ...
                x_list.append(x)
        # print("after trans1", [x_list[i].shape for i in range(len(x_list))])
        y_list = self.stage2(x_list)
        # print("after stage2", [y_list[i].shape for i in range(len(y_list))])
        # print("after stage2", y_list[0].shape, len(y_list))
        x_list = []
        ...
        # print("final", [x[i].shape for i in range(len(x))])
        return x
