#!/bin/bash
# Evaluate a trained ucf checkpoint @ 22 CLASSES on all 7 degradation levels.
# Batch size 8, as in the IAB reference script training/scripts_test/ucf.bash.
#
# Submit: sbatch comparison/training/scripts/cineca_ucf_test.sh [ckpt]
# Without an argument it picks the most recent run's ckpt_best.pth.
#SBATCH --account=EUHPC_D35_189
#SBATCH --partition=boost_usr_prod
#SBATCH --job-name=iab_ucf_test
#SBATCH --nodes=1
...
CKPT="${1:-}"
if [ -z "$CKPT" ]; then
  CKPT=$(ls -t comparison/training/logs/default_split/ucf/*/ckpt_best.pth 2>/dev/null | head -1 || true)
fi
...
echo "Done. Results under comparison/training/logs_test/default_split/ucf/test_<ts>/"
