#!/bin/bash
# ============================================================================
# CINECA Leonardo — Evaluate a trained HiFi-Net attributor checkpoint.
# Tests degraded levels [level_start, level_end). Use 0..7 for all 7 levels.
#
# Submit:  sbatch comparison/training/scripts/cineca_hifi_net_test.sh
# ============================================================================

#SBATCH --account=EUHPC_D26_009B
#SBATCH --partition=boost_usr_prod
#SBATCH --job-name=iab_hifi_test
#SBATCH --nodes=1
...
CKPT="${1:-}"
if [ -z "$CKPT" ]; then
  CKPT=$(ls -t comparison/training/logs/default_split/hifi_net/*/ckpt_best.pth 2>/dev/null | head -1 || true)
fi
