"""Train a spherical cosine classifier with CLIP, LoRA and a shared MLP.

Image and text-anchor embeddings are L2-normalized. Cross-entropy uses their
cosine similarities scaled by a learned inverse temperature. Class prompts are
fixed strings, but their embeddings change through the trainable model.
Caption tensors do not enter the objective.

Supports a balanced sampler, manifest or internal train/validation splitting,
and best-balanced-validation checkpointing.
Match data, capacity and training options explicitly when comparing runs with
the hyperbolic trainer; the two trainers have different loss and configuration
options.

Usage: python train_attribution_euclidean.py --help
"""
import argparse
...
# Share class ordering, prompt construction and balanced-sampler implementation.
...
def parse_args():
    ...
    p.add_argument("--logit_scale_init", type=float, default=2.6593,
                   help="Initial log inverse temperature (default log(1/0.07)).")
    ...
    p.add_argument("--split_manifest", default=None,
                   help="Harness split JSON containing train/val/test path lists. "
                        "Without it, use the dataset's internal caption-based split. "
                        "Match split and training settings across compared models.")
...
def main():
    ...
    # Datasets: manifest membership or the dataset's internal split.
    common = dict(
    ...
    if args.split_manifest:
        # Select manifest train/val rows without caption filtering.
        import json
