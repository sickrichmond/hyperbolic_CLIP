...
def build_family_anchors(class_names: list[str], hierarchy: str,
    ...
    """Return sorted family names, class-to-family indices, and family prompts.

    Read HiFi-Net level 3 or a JSON class-to-family map for the emergent option."""
    if hierarchy == "hifi":
...
class AnchorState:
    """Class/family anchor tensors and axis apertures used by training.
    ...
    def __init__(self, model, train_ds, class_names, anchor_texts, args, device):
        ...
        self.fam_ids = self.fam_mask = self.family_of = None
        self.family_names: list[str] = []
        if args.hierarchy != "none":
            self.family_names, fam_idx, fam_texts = build_family_anchors(
                class_names, args.hierarchy, args.hierarchy_json)
            fam_tok = tokenizer(fam_texts, return_tensors="pt", padding="max_length",
                                truncation=True, max_length=77)
            self.fam_ids  = fam_tok["input_ids"].to(device)
            self.fam_mask = fam_tok["attention_mask"].to(device)
            self.family_of = torch.tensor(fam_idx, dtype=torch.long, device=device)
            print(f"Hierarchy '{args.hierarchy}': {len(self.family_names)} families")
            for f, name in enumerate(self.family_names):
