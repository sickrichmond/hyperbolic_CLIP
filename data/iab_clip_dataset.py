...
Samples contain processed pixels, caption tokens and class metadata.
Caption templates encode visual content and the attribution label:
...
class IABCLIPDataset(Dataset):
    ...
    def __init__(
        ...
        """Build samples with caption lookup and deterministic internal splits.

        In "caption" mode, split by (generator, semantic). require_caption
        filters eligible rows; otherwise images without captions are eligible.
        include_uncaptioned additionally admits uncaptioned rows to val/all
        when caption filtering is enabled.

        In "stratified" mode, pool and shuffle each generator across semantics,
        then partition train/val/test. This branch filters training rows without
        captions regardless of require_caption; val/test retain them.
        max_per_class caps each generator here, versus each generator/semantic
        pair in "caption" mode.

        Nonempty include_paths/exclude_paths filter before internal splitting.
        Internal splits are not the comparison manifest: use its allowlists
        with split="all" for exact manifest membership.
        """
        ...
        if self.split_scheme == "stratified":
            # Pool by generator, shuffle deterministically, then partition.
            ...
            for gen in generators:
                ...
                kept = 0
                for p, sem in part:
                    # This branch filters uncaptioned training rows regardless of require_caption.
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # Retry transient file-access errors on the same image path.
        img = open_image_retry(img_path)

        # Apply the benchmark's grok3 crop: remove 100 px from the right
        # and 50 px from the bottom before preprocessing.
        if generator == "grok3":
            w, h = img.size
            if w > 100 and h > 50:
