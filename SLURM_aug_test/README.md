...
Each `eval_*.sbatch` file is a four-task array. Array indices map to the four
dataset names in `datasets.txt`; each task evaluates every valid image in its
dataset and writes `metrics.json` plus `summary.csv` below:
